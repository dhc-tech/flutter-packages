...
void main() {
  ...
  setUp(() {
    ...
    runner = CommandRunner<void>(
      'make-deps-path-based_command',
      'Test for $MakeDepsPathBasedCommand',
    );
  ...
  void addDependencies(
    ...
    lines.insertAll(dependenciesStartIndex + 1, <String>[
      for (final String dependency in dependencies) '  $dependency: $constraint',
    ]);
...
''');
  ...
  Map<String, String> getDependencyOverrides(RepositoryPackage package) {
    ...
    return pubspec.dependencyOverrides.map(
      (String name, Dependency dep) =>
          MapEntry<String, String>(name, (dep is PathDependency) ? dep.path : dep.toString()),
    );
  }

  test('no-ops for no plugins', () async {
    createFakePackage('foo', packagesDir, isFlutter: true);
    final RepositoryPackage packageBar = createFakePackage('bar', packagesDir, isFlutter: true);
    addDependencies(packageBar, <String>['foo']);
    final String originalPubspecContents = packageBar.pubspecFile.readAsStringSync();

    final List<String> output = await runCapturingPrint(runner, <String>['make-deps-path-based']);

    expect(output, containsAllInOrder(<Matcher>[contains('No target dependencies')]));
  ...
  test('includes explanatory comment', () async {
    final RepositoryPackage packageA = createFakePackage('package_a', packagesDir, isFlutter: true);
    createFakePackage('package_b', packagesDir, isFlutter: true);

    addDependencies(packageA, <String>['package_b']);

    await runCapturingPrint(runner, <String>[
      'make-deps-path-based',
      '--target-dependencies=package_b',
    ]);
  ...
  test('rewrites "dependencies" references', () async {
    final RepositoryPackage simplePackage = createFakePackage('foo', packagesDir, isFlutter: true);
    ...
    final RepositoryPackage pluginImplementation = createFakePlugin('bar_android', pluginGroup);
    final RepositoryPackage pluginAppFacing = createFakePlugin('bar', pluginGroup);

    addDependencies(simplePackage, <String>['bar', 'bar_android', 'bar_platform_interface']);
    addDependencies(pluginAppFacing, <String>['bar_platform_interface', 'bar_android']);
    addDependencies(pluginImplementation, <String>['bar_platform_interface']);
  ...
  test('rewrites "dev_dependencies" references', () async {
    ...
    final RepositoryPackage builderPackage = createFakePackage('foo_builder', packagesDir);

    addDevDependenciesSection(builderPackage, <String>['foo']);

    final List<String> output = await runCapturingPrint(runner, <String>[
      'make-deps-path-based',
      '--target-dependencies=foo',
    ]);
    ...
    final Map<String, String?> overrides = getDependencyOverrides(builderPackage);
  ...
  test('rewrites examples when rewriting the main package', () async {
    ...
    await runCapturingPrint(runner, <String>[
      'make-deps-path-based',
      '--target-dependencies=bar_android',
    ]);
  ...
  test('example overrides include both local and main-package dependencies', () async {
    ...
    addDependencies(pluginAppFacing, <String>['bar_platform_interface', 'bar_android']);
    addDependencies(pluginAppFacing.getExamples().first, <String>['another_package']);
  ...
  test('does not rewrite path-based dependencies that are already path based', () async {
    ...
    await runCapturingPrint(runner, <String>['make-deps-path-based', '--target-dependencies=foo']);
  ...
  test(
    ...
    () async {
      ...
      final RepositoryPackage targetPackage = createFakePackage('target', packagesDir);
      ...
      expect(
        ...
        matches(RegExp(r'dependency_overrides:.*a:.*b:.*c:.*', multiLine: true, dotAll: true)),
      );
    },
  );
  ...
  test('finds third_party packages', () async {
    ...
    final RepositoryPackage firstPartyPackge = createFakePlugin('foo', packagesDir);

    addDependencies(firstPartyPackge, <String>['bar']);

    final List<String> output = await runCapturingPrint(runner, <String>[
      'make-deps-path-based',
      '--target-dependencies=bar',
    ]);
    ...
    final Map<String, String?> simplePackageOverrides = getDependencyOverrides(firstPartyPackge);
    expect(simplePackageOverrides.length, 1);
    expect(simplePackageOverrides['bar'], '../../third_party/packages/bar');
  });

  test('handles third_party target package references in third_party', () async {
    createFakePackage('bar', thirdPartyPackagesDir, isFlutter: true);
    final RepositoryPackage otherThirdPartyPackge = createFakePlugin('foo', thirdPartyPackagesDir);

    addDependencies(otherThirdPartyPackge, <String>['bar']);
    ...
    final Map<String, String?> simplePackageOverrides = getDependencyOverrides(
      otherThirdPartyPackge,
    );
    expect(simplePackageOverrides.length, 1);
    expect(simplePackageOverrides['bar'], '../../../third_party/packages/bar');
  });
  ...
  test('running a second time no-ops without failing', () async {
    ...
    final String simplePackageUpdatedContent = simplePackage.pubspecFile.readAsStringSync();
    final String appFacingPackageUpdatedContent = pluginAppFacing.pubspecFile.readAsStringSync();
    final String implementationPackageUpdatedContent = pluginImplementation.pubspecFile
        .readAsStringSync();
    final List<String> output = await runCapturingPrint(runner, <String>[
      'make-deps-path-based',
      '--target-dependencies=bar,bar_platform_interface',
    ...
    expect(simplePackage.pubspecFile.readAsStringSync(), simplePackageUpdatedContent);
    expect(pluginAppFacing.pubspecFile.readAsStringSync(), appFacingPackageUpdatedContent);
    expect(
  ...
  test('sorts with existing overrides', () async {
    ...
    addDependencies(pluginAppFacing, <String>['bar_platform_interface', 'bar_android']);
    addDependencies(pluginImplementation, <String>['bar_platform_interface']);
    addDependencyOverridesSection(simplePackage, <String>[
      'bar_android',
  ...
  group('target-dependencies-with-non-breaking-updates', () {
    test('no-ops for no published changes', () async {
      ...
      gitProcessRunner.mockProcessesForExecutable['git-show'] = <FakeProcessInfo>[
        FakeProcessInfo(MockProcess(stdout: package.pubspecFile.readAsStringSync())),
      ];
    ...
    test('includes bugfix version changes as targets', () async {
      ...
      final RepositoryPackage package = createFakePackage('foo', packagesDir, version: newVersion);

      final File pubspecFile = package.pubspecFile;
      final String changedFileOutput = <File>[pubspecFile].map((File file) => file.path).join('\n');
      gitProcessRunner.mockProcessesForExecutable['git-diff'] = <FakeProcessInfo>[
        FakeProcessInfo(MockProcess(stdout: changedFileOutput)),
      ];
      final String gitPubspecContents = pubspecFile.readAsStringSync().replaceAll(
        newVersion,
        '1.0.0',
      );
      // Simulate no change to the version in the interface's pubspec.yaml.
      ...
      expect(output, containsAllInOrder(<Matcher>[contains('Rewriting references to: foo...')]));
    });
    ...
    test('does not update references with an older major version', () async {
      ...
      final RepositoryPackage targetPackage = createFakePackage(
        'foo',
        packagesDir,
        version: newVersion,
      );
      final RepositoryPackage referencingPackage = createFakePackage('bar', packagesDir);

      // For a dependency on ^1.0.0, the 2.0.0->2.0.1 update should not apply.
      addDependencies(referencingPackage, <String>['foo'], constraint: '^1.0.0');
    ...
    test('does update references with a matching version range', () async {
      ...
      // For a dependency on ^1.0.0, the 2.0.0->2.0.1 update should not apply.
      addDependencies(referencingPackage, <String>['foo'], constraint: '^2.0.0');
      ...
      expect(output, containsAllInOrder(<Matcher>[contains('Rewriting references to: foo')]));
      expect(referencingPubspec.dependencyOverrides['foo'] is PathDependency, true);
    });
    ...
    test('skips anything outside of the packages directory', () async {
      ...
      expect(
        output,
        containsAllInOrder(<Matcher>[
          contains('Skipping /tool/flutter_plugin_tools/pubspec.yaml; not in packages directory.'),
          contains('No target dependencies'),
        ]),
      );
    });
