...
#define APPLE_SIGN_IN_PLUGIN_LINUX(obj)                                     \
  (G_TYPE_CHECK_INSTANCE_CAST((obj), apple_sign_in_plugin_linux_get_type(), \
                              AppleSignInPluginLinux))
...
G_DEFINE_TYPE(AppleSignInPluginLinux, apple_sign_in_plugin_linux,
              g_object_get_type())
...
static void handle_native_callback(AppleSignInPluginLinux* self,
                                   const gchar* callback_url,
                                   FlMethodCall* method_call) {
  if (self->pending_method_call == nullptr) {
    g_autoptr(FlValue) ack = fl_value_new_bool(FALSE);
    fl_method_call_respond(
        method_call, FL_METHOD_RESPONSE(fl_method_success_response_new(ack)),
        nullptr);
    return;
  }
  ...
  if (std::chrono::duration_cast<std::chrono::minutes>(now -
          ...
          .count() >= 5) {
    fl_method_call_respond(
        self->pending_method_call,
        FL_METHOD_RESPONSE(fl_method_error_response_new(
            "canceled", "Sign in with Apple timed out waiting for callback.",
            nullptr)),
        nullptr);
  ...
  if (self->expected_scheme != nullptr) {
    g_autofree gchar* scheme_prefix =
        g_strdup_printf("%s://", self->expected_scheme);
    if (!g_str_has_prefix(callback_url, scheme_prefix)) {
      fl_method_call_respond(
          self->pending_method_call,
          FL_METHOD_RESPONSE(fl_method_error_response_new(
              "authorization_failed",
              "Apple Sign-In callback URI scheme mismatch.", nullptr)),
          nullptr);
      cleanup_pending(self);
      g_autoptr(FlValue) ack = fl_value_new_bool(FALSE);
      fl_method_call_respond(
  ...
  if (self->expected_host != nullptr && strlen(self->expected_host) > 0) {
    g_autofree gchar* host_prefix =
        g_strdup_printf("%s://%s", self->expected_scheme, self->expected_host);
    if (!g_str_has_prefix(callback_url, host_prefix)) {
      fl_method_call_respond(
          self->pending_method_call,
          FL_METHOD_RESPONSE(fl_method_error_response_new(
              "authorization_failed",
              "Apple Sign-In callback URI host mismatch.", nullptr)),
          nullptr);
      cleanup_pending(self);
      g_autoptr(FlValue) ack = fl_value_new_bool(FALSE);
      fl_method_call_respond(
  ...
  fl_value_set_string_take(result_map, "callbackUrl",
                           fl_value_new_string(callback_url));
  fl_value_set_string_take(
      result_map, "expectedState",
      fl_value_new_string(self->pending_state != nullptr ? self->pending_state
                                                         : ""));

  fl_method_call_respond(
      self->pending_method_call,
      FL_METHOD_RESPONSE(fl_method_success_response_new(result_map)), nullptr);
  cleanup_pending(self);
...
static void apple_sign_in_plugin_linux_handle_method_call(
    AppleSignInPluginLinux* self, FlMethodCall* method_call) {
  ...
  if (strcmp(method, "isAvailable") == 0) {
    ...
    if (self->pending_method_call != nullptr) {
      response = FL_METHOD_RESPONSE(fl_method_error_response_new(
          "already_in_progress",
          "A Sign in with Apple request is already in progress.", nullptr));
      fl_method_call_respond(method_call, response, nullptr);
      return;
    }
    ...
    if (url_val == nullptr || state_val == nullptr || scheme_val == nullptr) {
      response = FL_METHOD_RESPONSE(fl_method_error_response_new(
          "invalid_arguments",
          "Missing required url, state, or callbackScheme argument.", nullptr));
      fl_method_call_respond(method_call, response, nullptr);
      return;
    }
    ...
    self->expected_host = host_val != nullptr && fl_value_get_type(host_val) ==
    ...
    if (cb_val != nullptr &&
        fl_value_get_type(cb_val) == FL_VALUE_TYPE_STRING) {
      handle_native_callback(self, fl_value_get_string(cb_val), method_call);
    } else {
      response = FL_METHOD_RESPONSE(fl_method_error_response_new(
          "invalid_arguments", "Missing 'callbackUrl'.", nullptr));
      fl_method_call_respond(method_call, response, nullptr);
    }
    ...
    if (self->pending_method_call != nullptr) {
      fl_method_call_respond(
          self->pending_method_call,
          FL_METHOD_RESPONSE(fl_method_error_response_new(
              "canceled", "Sign in with Apple was canceled.", nullptr)),
          nullptr);
      cleanup_pending(self);
    }
    g_autoptr(FlValue) ack = fl_value_new_bool(TRUE);
...
static void apple_sign_in_plugin_linux_class_init(
    AppleSignInPluginLinuxClass* klass) {
...
void apple_sign_in_plugin_linux_register_with_registrar(
  ...
  plugin->channel = fl_method_channel_new(
      ...
      "apple_sign_in_plugin_linux", FL_METHOD_CODEC(codec));
