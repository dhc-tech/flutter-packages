...
AppleSignInPluginWindows::~AppleSignInPluginWindows() { CleanupPending(); }
...
void AppleSignInPluginWindows::HandleMethodCall(
  ...
  if (method_call.method_name().compare("isAvailable") == 0) {
    ...
    if (url_it == arguments->end() || state_it == arguments->end() ||
        scheme_it == arguments->end()) {
      result->Error("invalid_arguments",
                    "Missing required url, state, or callbackScheme argument.");
      return;
    }
    ...
    if (host_it != arguments->end() &&
        std::holds_alternative<std::string>(host_it->second)) {
    ...
    if ((INT_PTR)hResult <= 32) {
      if (pending_result_) {
        pending_result_->Error(
            "launch_failed",
            "Failed to open system browser with ShellExecute.");
      }
      CleanupPending();
    }
...
void AppleSignInPluginWindows::ProcessNativeCallback(
  ...
  if (!expected_scheme_.empty()) {
    ...
    if (callback_url.rfind(expected_prefix, 0) != 0) {
      pending_result_->Error("authorization_failed",
  ...
  resultMap[flutter::EncodableValue("callbackUrl")] =
      flutter::EncodableValue(callback_url);
  resultMap[flutter::EncodableValue("expectedState")] =
      flutter::EncodableValue(pending_state_);
