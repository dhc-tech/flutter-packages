...
String _createTestJwt({
  ...
  final h = toBase64Url(header);
  final p = toBase64Url(payload);
  final s = base64Url.encode(utf8.encode('mock_signature')).replaceAll('=', '');
  return '$h.$p.$s';
}
...
void main() {
  group('JwtDecoder Tests', () {
    ...
    test('getExpirationDate and getRemainingTime calculate correctly', () {
      ...
      final tokenTime = JwtDecoder.getTokenTime(token);
