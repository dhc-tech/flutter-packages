...
void main() {
  // Unlike apple_sign_in_plugin_darwin (which talks to
  // ASAuthorizationAppleIDProvider directly and needs no config), Android
  // has no native Apple Sign-In API — it goes through Apple's web
  // authorization flow via Chrome Custom Tabs, so it needs your Apple
  // Services ID and redirect URI set before signIn() can be called.
  AppleSignInPlatform.instance = AppleSignInAndroidImpl()
    ..config = const AppleSignInAndroidConfig(
