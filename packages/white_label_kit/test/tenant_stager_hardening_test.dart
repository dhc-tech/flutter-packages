...
void main() {
  ...
  test('stage() rejects a hand-built TenantConfig with a path-traversal asset', () {
    ...
    File(p.join(projectRoot.path, 'secret_outside', 'leak.png')).writeAsStringSync('TOP_SECRET');
  ...
  test('stage() is atomic: a failed re-stage leaves the prior good output untouched', () {
    ...
    expect(
      File(p.join(stager.stagingDirFor('acme'), 'assets', 'logo.png')).readAsStringSync(),
      'LOGO_V1',
    );
  });

  test('stage() rejects two different declared assets that collide on basename', () {
    final acmeAssets = Directory(p.join(projectRoot.path, 'tenants', 'acme', 'assets'))
      ..createSync(recursive: true);
    final iconSubdir = Directory(p.join(projectRoot.path, 'tenants', 'acme', 'icon'))
      ..createSync(recursive: true);
    File(p.join(acmeAssets.path, 'logo.png')).writeAsStringSync('LOGO_CONTENT');
    // Different source file, but same basename as the logo once staged.
    File(p.join(iconSubdir.path, 'logo.png')).writeAsStringSync('ICON_CONTENT_DIFFERENT_FILE');
