...
class CreateProjectCommand extends Command<void> {
  ...
  @override
  final description = 'Creates a new Flutter project from a template and sets up signing.';

  @override
  Future<void> run() async {
    kLog('\n🚀 CREATE PROJECT FROM TEMPLATE');

    _githubRepo = argResults?['github-repo'] as String? ?? 'Digvijaysinh2204/dig_template';
    ...
    if (bundleId == null ||
        bundleId.isEmpty ||
        !RegExp(r'^[a-zA-Z][a-zA-Z0-9_]*(\.[a-zA-Z][a-zA-Z0-9_]*)*$').hasMatch(bundleId)) {
      kLog(
        ...
        () async {
          ...
          for (final file in archive) {
            ...
            if (file.isFile) {
              ...
              Directory(p.join(tempDir.path, filename)).createSync(recursive: true);
            }
          }
