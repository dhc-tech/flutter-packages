...
class InteractiveMenu {
  ...
  /// Shows the interactive dashboard loop until the user exits.
  Future<void> show() async {
    while (true) {
      ...
      final String? response = stdin.readLineSync()?.trim();
      if (response == '0') {
        exit(0);
      }
  ...
  Future<void> _buildReleaseMenu() async {
    while (true) {
      ...
      _drawPrompt('1-3');
      final String? r = stdin.readLineSync()?.trim();
      if (r == '0' || r == null || r.isEmpty) {
        return;
      }
    ...
    while (true) {
      ...
      switch (r) {
        ...
        case '2':
          ...
          await _pause();
        case '3':
          final CommandRunner<dynamic> pubRunner = CommandRunner('dg', 'temp')
  ...
  Future<void> _projectMenu() async {
    while (true) {
      ...
      if (isFlutter) {
        switch (r) {
          case '1':
            final String? name = _promptUser('New Project Name');
            ...
            await _pause();
          case '2':
            final String? name = _promptUser('New Module Name');
            if (name == null || name.isEmpty) {
              break;
            }
            ...
            await _pause();
          case '3':
            final String? name = _promptUser('Module Name to Remove');
            if (name == null || name.isEmpty) {
              break;
            }
            ...
            await _pause();
          case '4':
            final String? name = _promptUser('New display name (optional)');
            final String? bundle = _promptUser('New bundle ID (optional)');
            final args = ['rename'];
            if (name != null && name.isNotEmpty) {
              args.addAll(['--name', name]);
            }
  ...
  Future<void> _handleUpdateCheck(bool beta) async {
    kLog('  Checking for updates...');
    final String? latest = beta
    ...
    if (latest != null && VersionUtils.isNewer(latest, kDigCliVersion)) {
      ...
      if (stdin.readLineSync()?.trim().toLowerCase() == 'y') {
        final String verArg = beta ? latest : '';
        await Process.start(
            'dart',
            [
              'pub',
...
/// Launches the interactive dashboard menu.
