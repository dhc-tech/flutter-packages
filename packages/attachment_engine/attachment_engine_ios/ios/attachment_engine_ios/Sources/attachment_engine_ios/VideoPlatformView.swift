...
/// Replaces `video_player`. Embeds an `AVPlayer`-backed `AVPlayerLayer`
/// through a `FlutterPlatformView` (`UiKitView` on the Dart side),
/// mirroring the Android `TextureView`/`MediaPlayer` implementation:
/// control (`load`/`play`/`pause`/`seek`/`setSpeed`/`setVolume`/`dispose`)
/// goes over `attachment_engine/video`, and playback/buffering events go
/// over `attachment_engine/video_events/{playerId}`.
...
class VideoPlatformViewFactory: NSObject, FlutterPlatformViewFactory {
  ...
  func create(
    ...
    return VideoPlatformViewImpl(frame: frame, entry: videoChannel.entry(for: playerId))
  }
...
private class VideoPlatformViewImpl: NSObject, FlutterPlatformView {
  private let containerView: UIView
  private let playerLayer: AVPlayerLayer

  init(frame: CGRect, entry: VideoPlayerEntry) {
