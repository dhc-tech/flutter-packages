...
/// Replaces `pdfx`. Uses PDFKit (`PDFDocument`, `PDFPage.thumbnail`) to
/// open a PDF and render pages to PNG bitmaps, keyed by an opaque handle
/// string so the Dart side can hold multiple documents open concurrently.
///
...
class PdfChannel: NSObject, PdfHostApi {
    ...
    -> FlutterStandardTypedData
  {
    ...
    #else
      throw PigeonError(code: "render_failed", message: "Unsupported platform", details: nil)
    #endif
  }
