...
/// Downloads over `URLSession`/`URLSessionDownloadTask`, reporting
/// progress/completion/error over a shared EventChannel
/// (`attachment_engine/download_events`), each event tagged with its
/// `downloadId`. Identical logic to the iOS implementation
/// (Foundation-only, no UIKit/AppKit).
///
