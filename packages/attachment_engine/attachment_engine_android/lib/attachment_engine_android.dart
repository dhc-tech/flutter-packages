...
/// The Android implementation of [AttachmentEnginePlatform].
///
...
/// lockstep with the native Kotlin implementations of those interfaces
/// under `android/src/main/kotlin/com/dhc/attachment_engine/`.
///
...
class AttachmentEngineAndroid extends AttachmentEnginePlatform {
  /// Registers this class as the default instance of [AttachmentEnginePlatform].
  static void registerWith() {
    AttachmentEnginePlatform.instance = AttachmentEngineAndroid();
  }
  ...
  /// Android has no in-app Office document viewer analogous to iOS's
  /// QuickLook (there is no [OfficeHostApi] implementation registered on
  /// the Android native side). This is a documented platform limitation,
  /// not a bug: gracefully degrade to the existing external-open flow
  /// (`ACTION_VIEW` + `FileProvider`) instead of calling the unimplemented
  /// native API, so callers can invoke [openOfficePreview] unconditionally
  /// across platforms.
  @override
