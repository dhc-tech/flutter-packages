...
/**
 * Replaces `video_player`. Embeds a [TextureView] backed by
 * [android.media.MediaPlayer] (Media3/ExoPlayer would be preferable for
 * adaptive streaming, but was intentionally avoided here to keep this
 * pass free of additional third-party Gradle dependencies — see README
 * "Known limitations": no adaptive bitrate/DASH/HLS support).
 */
...
/**
 * Shared registry so the control [MethodChannel] and the [PlatformView]
 * factory (created lazily by the Flutter engine when the widget attaches)
 * both operate on the same native player instance for a given `playerId`.
 */
/**
 * Method calls (load/play/pause/seek/setSpeed/setVolume/dispose) are
 * dispatched through the Pigeon-generated [VideoHostApi]. The platform-view
 * factory below stays hand-registered (it isn't a method call), and
 * playback-state *events* stay on a hand-written per-`playerId`
 * `EventChannel` for the same reason as [AudioChannel] — see the note in
