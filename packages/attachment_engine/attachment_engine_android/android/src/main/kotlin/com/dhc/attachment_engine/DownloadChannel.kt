...
/**
 * Replaces `dio`. Downloads over plain [HttpURLConnection] (avoiding third-party OkHttp), reporting
 * progress/completion/error over a shared EventChannel, each event tagged with its `downloadId`.
 *
 * `startDownload`/`resumeDownload`/`cancelDownload` are dispatched through the Pigeon-generated
 * [DownloadHostApi]. Progress/completion/error *events* stay on a hand-written shared
 * `EventChannel` — Pigeon's event-channel support models one stream per API, not the free-form,
...
class DownloadChannel : DownloadHostApi {
  ...
  override suspend fun startDownload(
      ...
      destPath: String
  ): String = beginDownload(url, headers, destPath, resume = false)

  override suspend fun resumeDownload(
      url: String,
      headers: Map<String, String>,
      destPath: String
  ): String = beginDownload(url, headers, destPath, resume = true)
  ...
  private fun beginDownload(
    ...
    executor.submit {
      ...
      try {
        ...
        val total =
            if (append && contentLength > 0) contentLength + existingBytes else contentLength
        ...
        emit(
            mapOf(
                ...
                "type" to "error",
                "message" to
                    if (diskFull) "insufficient_storage" else (e.message ?: "download failed"),
            ),
        )
      } catch (e: Exception) {
        emit(
