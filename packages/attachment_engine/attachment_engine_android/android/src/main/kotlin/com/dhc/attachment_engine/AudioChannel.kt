...
/**
 * Replaces `just_audio`. Uses [android.media.MediaPlayer] for both local and streaming remote audio
 * playback (a pragmatic simplification vs. Media3/ExoPlayer — see README "Known limitations": no
 * adaptive streaming, but sufficient for direct-URL MP3/AAC/etc. playback).
 *
 * Method calls (load/play/pause/seek/setSpeed/setVolume/dispose) are dispatched through the
 * Pigeon-generated [AudioHostApi]. Playback-state *events* stay on a hand-written per-`playerId`
 * `EventChannel`: Pigeon's event-channel support doesn't cleanly express a channel name keyed by an
