...
/**
 * AttachmentEnginePlugin: registers one Pigeon-generated `HostApi` (or, for the
 * audio/video/download event streams, one hand-written `EventChannel` — see
 * `attachment_engine_platform_interface/pigeons/messages.dart`) plus one platform-view factory, per
 * capability. Replaces the third-party plugins previously used (pdfx, video_player, just_audio,
 * share_plus, open_filex, dio, path_provider) with hand-written native implementations wired
 * through Pigeon codegen for the method-call surface. Webview embedding is handled by the official
 * `webview_flutter` package instead, at the app-facing layer — not through this plugin.
 */
class AttachmentEnginePlugin : FlutterPlugin, MethodCallHandler {
